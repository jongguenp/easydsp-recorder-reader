...
function renderFiles(){
 const el=$("files");if(!state.files.length){el.innerHTML="<div class=\"empty\">REC 파일을 선택하세요.</div>";return}
 el.innerHTML=state.files.map(f=>"<div class=\"file-row\"><input class=\"file-toggle\" type=\"checkbox\" data-file-vis=\""+f.id+"\" "+(state.fileVisible[f.id]!==false?"checked":"")+"><div><div class=\"file-name\"><b>"+f.name+"</b></div><div class=\"hint\">"+f.channels.length+" channels · "+(f.channels[0]?.y.length||0)+" samples <span class=\"persist-badge\">로컬 저장</span></div></div><div class=\"file-actions\"><button class=\"chip mini\" data-file-only=\""+f.id+"\">이 파일만</button><button class=\"link danger\" data-file-del=\""+f.id+"\">삭제</button></div></div>").join("");
 el.querySelectorAll("[data-file-vis]").forEach(x=>x.onchange=()=>{state.fileVisible[x.dataset.fileVis]=x.checked;refreshDataViews()});
 el.querySelectorAll("[data-file-only]").forEach(x=>x.onclick=()=>{const id=x.dataset.fileOnly;for(const f of state.files)state.fileVisible[f.id]=f.id===id;renderFiles();refreshDataViews()});
 el.querySelectorAll("[data-file-del]").forEach(x=>x.onclick=()=>removeFile(x.dataset.fileDel))
}
function refreshDataViews(){renderChannels();renderLegend();fillDiff();renderStats();renderRangeStats();draw();persistSettings()}
function renderChannels(){
 const list=$("channelList");if(!state.files.length){list.innerHTML="<div class=\"empty\">REC 파일을 먼저 열어주세요.</div>";return}
 list.innerHTML=state.files.map(f=>{const rows=f.channels.map(ch=>{const id=sid(f,ch),on=state.visible[id]!==false;return "<div class=\"channel\"><input type=\"checkbox\" data-vis=\""+id+"\" "+(on?"checked":"")+" "+(state.fileVisible[f.id]===false?"disabled":"")+"><div class=\"channel-main\"><div><div class=\"hint\">"+ch.name+"</div><input type=\"text\" data-alias=\""+ch.name+"\" value=\""+alias(ch.name)+"\"></div><input class=\"unit\" type=\"text\" data-unit=\""+ch.name+"\" value=\""+unit(ch.name)+"\" placeholder=\"단위\"></div><input type=\"color\" data-color=\""+ch.name+"\" value=\""+color(ch.name)+"\"></div>"}).join("");return "<div class=\"group\"><div class=\"group-head\"><div class=\"group-title\">"+f.name+"</div><div><button class=\"chip mini\" data-group-on=\""+f.id+"\">모두 ON</button> <button class=\"chip mini\" data-group-off=\""+f.id+"\">모두 OFF</button></div></div>"+rows+"</div>"}).join("");
 list.querySelectorAll("[data-vis]").forEach(x=>x.onchange=()=>{state.visible[x.dataset.vis]=x.checked;refreshDataViews()});
 list.querySelectorAll("[data-alias]").forEach(x=>x.onchange=()=>{state.aliases[x.dataset.alias]=x.value.trim()||x.dataset.alias;renderAll();persistSettings()});
 list.querySelectorAll("[data-unit]").forEach(x=>x.onchange=()=>{state.units[x.dataset.unit]=x.value.trim();renderAll();persistSettings()});
 list.querySelectorAll("[data-color]").forEach(x=>x.oninput=()=>{state.colors[x.dataset.color]=x.value;renderAll();persistSettings()});
 list.querySelectorAll("[data-group-on]").forEach(x=>x.onclick=()=>setGroup(x.dataset.groupOn,true));
 list.querySelectorAll("[data-group-off]").forEach(x=>x.onclick=()=>setGroup(x.dataset.groupOff,false))
}
function setGroup(fileId,on){const f=state.files.find(x=>x.id===fileId);if(!f)return;if(on)state.fileVisible[fileId]=true;for(const ch of f.channels)state.visible[sid(f,ch)]=on;renderFiles();refreshDataViews()}
function setAll(on){if(on)for(const f of state.files)state.fileVisible[f.id]=true;for(const f of state.files)for(const ch of f.channels)state.visible[sid(f,ch)]=on;renderFiles();refreshDataViews()}
...
function fillDiff(){const a=base();for(const id of["a","b"])$(id).innerHTML=a.map(s=>`<option value="${s.id}">${s.file} · ${label(s)}</option>`).join("")}
...
function renderStats(){const v=visibleSeries();$("stats").innerHTML=v.length?v.map(s=>{const q=statsRange(s);return`<tr><td>${label(s)}<br><small>${s.file}</small></td><td>${fmt(q.min)}</td><td>${time(q.mi).toFixed(4)} s</td><td>${fmt(q.max)}</td><td>${time(q.ma).toFixed(4)} s</td><td>${fmt(q.mean)}</td><td>${fmt(q.rms)}</td></tr>`}).join(""):'<tr><td colspan="7">표시 데이터가 없습니다.</td></tr>'}
...
function renderCursorSummary(){
 ...
 if(state.cursorA!=null&&state.cursorB!=null){const dt=Math.abs(time(state.cursorB)-time(state.cursorA));lines.push(`Δt: ${dt.toFixed(6)} s`);for(const s of v){const a=s.y[Math.min(state.cursorA,s.y.length-1)],b=s.y[Math.min(state.cursorB,s.y.length-1)];lines.push(`${label(s)} ΔY: ${fmt(b-a)}`)}}
